...
## ⚠️ Совместимость с версией Remnawave

Текущие версии Remnawave **убрали поле `uuid` у юзера** — идентификатор теперь
числовой `id` (REST: `GET/PATCH /api/users/:userId`), плюс отдельно `shortUuid`
и `vlessUuid`. И внешний сквад — **одно поле `externalSquadUuid`** на юзере
(nullable строка), а не список `activeExternalSquads`, как было раньше.

`remnawave.py` это учитывает:
- `RemnawaveAPI._normalize_user()` подставляет `uuid = str(id)` во все объекты
  юзеров сразу при получении — остальной код (`monitor.py`, `bot.py`,
  `database.py`, ...) как и раньше работает через `user["uuid"]`, менять
  ничего не пришлось.
- Смена сквада — `PATCH /api/users` с `{"id": <номер>, "externalSquadUuid": ...}`
  (сверено с рабочим кодом [Bedolaga-бота](https://github.com/BEDOLAGA-DEV/remnawave-bedolaga-telegram-bot),
  который ходит в ту же панель тем же способом).
- **Важно:** раз слот один, юзер физически не может одновременно быть в
  limited-сквадах двух разных нод. Если лимит превышен сразу на нескольких
  нодах — `set_user_external_squads()` берёт первый таргет и пишет warning в
  лог про остальные. Для большинства конфигов (один limited-сквад на всех
  нод) это не проблема; если у тебя разные сквады по нодам — учитывай при
  проектировании политики.

Если твоя панель — старой версии с полем `uuid` и списком
`activeExternalSquads`, ничего дополнительно настраивать не нужно:
`_normalize_user` трогает объект только когда `uuid` отсутствует.
...
### 6. Режим применения: `manual` (по умолчанию) / `auto`

`enforcement_mode` (`config.json` или из бота, override в `kv_settings`):

| Режим | После гистерезиса |
|-------|-------------------|
| `manual` | Монитор **не блокирует**. Заводит заявку в `pending_approvals` и шлёт в приват + топик сообщение с кнопками `[🔒 Заблокировать] [⏭ Пропустить цикл] [🛡 В whitelist]`. Ждёт админа. |
| `auto` | Блокирует сразу (прежнее поведение). |

- `⏭ Пропустить цикл` → `status=skipped`, юзер освобождён до конца своего цикла, повторно не спрашиваем.
- `🔒` из бота сразу переключает сквад и пишет `limited_users` (period_key/billing_reset_at берутся из заявки).
- `dry_run=true` перекрывает оба режима (ничего не делает).
- Заявки за прошлый цикл чистятся автоматически (`purge_stale_approvals`).
- Бот: пункт `⏳ Заявки (N)` — список ожидающих; `📊 Статус` показывает режим и число заявок.
- `🎯 Проверка циклов` помечает такие строки как `АПРУВ?`.
...
📊 Статус         — dry-run, режим (manual/auto), кол-во limited/pending/заявок
⏳ Заявки (N)     — ожидающие апрув блокировки: [🔒][⏭][🛡] по каждой
...
| ✅ Апрув блокировок через бота | Готово | `enforcement_mode: manual` + `pending_approvals` |
| ⏳ Настройка тюнеров из бота | В плане | `⚙️ Настройки`: DB-слой поверх config.json |
